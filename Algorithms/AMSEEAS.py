...
RefTrials = None
svm_regressor = None
gaussian_regressor_1 = None
forest_regressor = None
gaussian_regressor_2 = None


def amseeas(n, m, xmin, xmax, fn, maxeval, ftol=10 ** (-6), pmut=0.1, beta=2, maxclimbs=3):
    ...
    global S, we, coef, xlow_help, xup_help, RefTrials, record_Ypred, record_dist
    global gaussian_regressor_1, gaussian_regressor_2, svm_regressor, forest_regressor
    ...
    while neval <= maxeval:
        ...
        print(f"\nFE = {neval}  BestValue = {fpop.min():.5f}  maxpopf={fpop.max():.2f}\n")
        print("================================================================================\n")
    ...
    print(f"\n=============================================")
    print(f"AMSEEAS execution lasted {duration:.2f} seconds")
    print(f"=============================================\n")
    print(f"BestValue={BestValue}")

    return BestValue, BestPar, NumIter, NumfEval, Ftolpop, fmin, fmax, fanaiter, temptemper, S, Y


# I FIXED THE RBF - IT IS THE SAME AS THE MATLAB VERSION NOW
...
def AF_EAS(ext_S, candidate, coefs, w):  # opou S o population ( m x n ) kai candidate to neo shmeio (1 x n )
    global record_Ypred, record_dist  # to candidate einai 2d array px np.array([[1, 2, 3]])
