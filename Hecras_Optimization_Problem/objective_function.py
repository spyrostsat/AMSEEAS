def hec_ras(new_solution):
    ...
    lateral_width = 1  # meters (tha aplopoih8ei kata thn kanonikopoihsh twn timwn - den paizei rolo sto telos)
    cost = 1  # euros per m3 of additional lateral structure (tha aplopoih8ei kata thn kanonikopoihsh twn timwn - den paizei rolo sto telos)
    ...
    counter_laterals = -1  # there are 27 (0 to 26) total lateral structures. At first, none of them is found, so the
    ...
    interested = False  # this variable determines whether i am in a region of interest (i.e. in file lines where
    # the lateral weirs are defined) or not

    counter = 0  # this counter counts which line of the geometry file (.g16true) I am currently reading
    ...
    hec.QuitRas()  # this command closes the hec-ras window and also stops the simulation if its not already completed
    ...
    for i in range(len(pathnames)):
        ...
        values = ts.values
        total_overflow += max(values.sum(),
                              0) * 60 * 5  # the values are expressed in terms of m3/s and refer to a 5MIN plan, so we multiply those by first by 60 (seconds) and then by 5 (min) to get the total overflow in m3.
        total_cost += laterals_lengths[i] * new_solution[i] * lateral_width * cost

    fid.close()

    max_overflow = 3272571.5  # the 'do nothing' solution (i.e. 0m elevation in all lateral structures) results in a total overflow equal to 3272571.5 m3 and 0 cost
    ...
    of = ((total_overflow / max_overflow) + (total_cost / max_cost)) / 2  # of values range from 0 to 1
